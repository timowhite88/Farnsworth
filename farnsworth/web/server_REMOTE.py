...
import uvicorn
# Autonomous improvements integration
from farnsworth.core.swarm_memory_bridge import (
    get_swarm_memory_bridge, initialize_swarm_memory_bridge, hook_swarm_interaction
)
from farnsworth.core.airllm_swarm import (
    get_airllm_swarm, initialize_airllm_swarm, queue_background_task
)
from farnsworth.core.smart_turn_taking import (
    get_turn_manager, select_next_speaker_smart, signal_turn_complete
)
...
def extract_ollama_content(response, max_length: int = 800) -> str:
    """
...
# ═══════════════════════════════════════════════════════════════════════════════
# ██████  DO NOT TOUCH - WORKING SWARM CHAT SYSTEM  ██████
# This code handles human-to-human visibility, bot mentions, and message broadcasting.
# Changes here can break the swarm chat for all users.
# Last verified working: 2026-01-29 by Opus
# ═══════════════════════════════════════════════════════════════════════════════
class SwarmChatManager:
    ...
    async def broadcast_user_message(self, user_id: str, content: str, user_name: str = None):
        """Broadcast a user message to all users and feed to learning engine."""
        user_name = user_name or self.user_names.get(user_id, "Anonymous")
        msg = {
            "type": "swarm_user",
            "user_id": user_id,
            "user_name": user_name,
    ...
    async def broadcast_bot_message(self, bot_name: str, content: str, is_thinking: bool = False):
        ...
        # Generate TTS audio URL for speaking bots (Farnsworth and Kimi)
        audio_url = None
        tts_bots = ["Farnsworth", "Kimi"]
        if not is_thinking and TTS_AVAILABLE and bot_name in tts_bots:
            try:
        ...
        # ═══════════════════════════════════════════════════════════════════════════
    # ██████  DO NOT TOUCH - WORKING TTS VOICE SYSTEM  ██████
    # This code handles Farnsworth and Kimi voice generation.
    # Changes here can break voice output for the swarm.
    # Last verified working: 2026-01-29 by Opus
    # ═══════════════════════════════════════════════════════════════════════════
    async def _generate_tts_async(self, text: str, text_hash: str, bot_name: str = "Farnsworth"):
        ...
        try:
            ...
            if tts_model:
                # Select voice based on bot - using consistent paths
                VOICE_FILES = {
                    "Kimi": "/workspace/Farnsworth/farnsworth/web/static/audio/kimi_voice.wav",
                    "Farnsworth": "/workspace/Farnsworth/farnsworth/web/static/audio/farnsworth_reference.wav"
                }
                reference_audio = VOICE_FILES.get(bot_name, VOICE_FILES["Farnsworth"]); logger.info(f"TTS: Using voice reference: {reference_audio} for {bot_name}")
                
                # Verify voice file exists
                if not Path(reference_audio).exists():
                    logger.warning(f"Voice file not found for {bot_name}: {reference_audio}")
                    reference_audio = "/workspace/Farnsworth/farnsworth/web/static/audio/farnsworth_reference.wav"
    ...
    async def _broadcast(self, message: dict):
        ...
        logger.info(f"_broadcast: sending {message.get('type')} from {message.get('bot_name', 'user')} to {len(self.connections)} users")

        # Hook for SwarmMemoryBridge - capture interactions for archival
        try:
            # Record activity for AirLLM idle detection
            airllm = get_airllm_swarm()
            if airllm:
                airllm.record_main_activity()
        except:
            pass
            
        try:
            if message.get("type") in ["swarm_bot", "swarm_message"]:
                await hook_swarm_interaction({
                    "speaker": message.get("bot_name") or message.get("user_name", "user"),
                    "content": message.get("content", ""),
                    "role": "assistant" if message.get("type") == "swarm_bot" else "user",
                    "timestamp": message.get("timestamp", datetime.now().isoformat())
                })
        except Exception as e:
            logger.debug(f"Memory hook error (non-critical): {e}")
        dead = []
...
# Feature awareness for all bots
SWARM_FEATURES = """
SWARM FEATURES (All bots know this):
- Users can mention you by name: @Claude, Hey Kimi, Farnsworth:, etc.
- When mentioned, respond directly and helpfully to that user
- You can delegate tasks to other bots if they're better suited (code->Claude, crypto->Swarm-Mind)
- When delegating, tell the user who's taking over and why
- Farnsworth and Kimi have TTS voices - they speak aloud
- Everyone can see everyone's messages in the swarm chat
- Take turns speaking - don't interrupt others while they're responding
- Keep responses complete but concise (under 150 words usually)
"""
...
def estimate_speaking_time(content: str, has_tts: bool = False) -> float:
    ...
    if has_tts:
        return base_time + 5  # Extra time for TTS (reduced with timeout fallback) generation + streaming
    return base_time


# ═══════════════════════════════════════════════════════════════════════════════
# ██████  DO NOT TOUCH - WORKING TURN-TAKING SYSTEM  ██████
# This code prevents bots from talking over each other.
# Changes here can cause overlapping audio and broken conversations.
# Last verified working: 2026-01-29 by Opus
# ═══════════════════════════════════════════════════════════════════════════════
async def wait_for_turn(speaker: str) -> bool:
    ...
    wait_count = 0
    max_wait = 45  # Max 45 seconds wait

    while True:
        # Check if we can proceed
        speaker_done = not _current_speaker or time.time() >= _speaking_until
        audio_done = not _awaiting_audio_complete

        if speaker_done and audio_done:
            break

        # Timeout - force release if waiting too long
        if wait_count > max_wait:
            logger.warning(f"{speaker} timeout waiting - forcing turn release")
            _awaiting_audio_complete = False  # Force clear
            _current_speaker = None
            break

        # Secondary timeout: if past speaking time, clear audio wait after 10 extra seconds
        # This handles case where no users are connected to send audio_complete signal
        if speaker_done and _awaiting_audio_complete and wait_count > 10:
            logger.info(f"Audio complete timeout - no clients connected, releasing turn")
...
def release_turn(speaker: str, content: str, has_tts: bool = False):
    ...
    async def delayed_release():
        ...
        global _current_speaker, _awaiting_audio_complete
        if _current_speaker == speaker:
            _current_speaker = None
            _awaiting_audio_complete = False  # Also clear audio wait
            logger.info(f"{speaker} turn released by timeout - next bot can speak")
...
async def autonomous_conversation_loop():
    ...
    # Initialize SwarmMemoryBridge for persistent conversation memory
    try:
        memory_bridge = await initialize_swarm_memory_bridge()
        if memory_bridge:
            await memory_bridge.start_background_consolidation()
            logger.info("  SwarmMemoryBridge: ENABLED - conversations being archived")
    except Exception as e:
        logger.warning(f"SwarmMemoryBridge init skipped: {e}")
                ...
                if farnsworth_turn:
                    ...
                    for bot in available_bots:
                        if bot == "Farnsworth":
                            ...
                            weights.append(1)
                    # Smart speaker selection (autonomous improvement integration)
                    last_msg = swarm_manager.chat_history[-1].get("content", "") if swarm_manager.chat_history else ""
                    speaker = select_next_speaker_smart(
                        last_speaker=recent_speakers[-1] if recent_speakers else None,
                        last_message=last_msg,
                        available=available_bots
                    ) or random.choices(available_bots, weights=weights, k=1)[0]
...
This is YOUR conversation - make it interesting."""
                ...
                try:
                    ...
                    if content and content.strip():
                        ...
                        release_turn(speaker, content, has_tts=(speaker in ["Farnsworth", "Kimi"]))
...
Be yourself. Make this conversation valuable."""
                            ...
                            try:
                                ...
                                if content and content.strip():
                                    ...
                                    release_turn(next_speaker, content, has_tts=(next_speaker in ["Farnsworth", "Kimi"]))
...
async def generate_swarm_responses(message: str, history: List[dict] = None):
    ...
    # Check for bot mentions (@Claude, Hey Kimi, etc.)
    mentioned_bot, cleaned_message = detect_bot_mention(message)
    if mentioned_bot:
        logger.info(f"Bot mention detected: {mentioned_bot}")
        response = await generate_mentioned_bot_response(mentioned_bot, cleaned_message, history)
        if response:
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        # Broadcast user message to swarm (redundant path for visibility)
        try:
            import hashlib
            msg_hash = hashlib.md5(f"api:{request.message[:50]}".encode()).hexdigest()[:8]
            await swarm_manager.broadcast_user_message(f"api_{msg_hash}", request.message, request.user_name or "Visitor")
        except Exception as e:
            logger.debug(f"Could not broadcast to swarm: {e}")
...
@app.get("/api/speak")
async def get_speak_audio(text_hash: str):
    """Serve cached audio by hash."""
    from pathlib import Path
    from fastapi.responses import FileResponse
    
    # Check local TTS cache
    cache_path = Path(f"/tmp/farnsworth_tts_cache/{text_hash}.wav")
    if cache_path.exists():
        return FileResponse(str(cache_path), media_type="audio/wav")
    
    # Check planetary audio shard
    audio_shard = get_planetary_audio_shard()
    if audio_shard:
        try:
            audio_data = audio_shard.get_audio(text_hash)
            if audio_data:
                temp_path = Path(f"/tmp/farnsworth_tts_cache/{text_hash}.wav")
...
# ═══════════════════════════════════════════════════════════════════════════════
# AirLLM Side Swarm - Background Processing API
# ═══════════════════════════════════════════════════════════════════════════════
...
async def airllm_queue_task(request: AirLLMTaskRequest):
    ...
    return {
        "success": True,
        "task_id": task_id,
        "message": f"Task queued for background processing",
...
@app.post("/api/airllm/activity")
async def airllm_record_activity():
    """Record main swarm activity (resets idle timer)."""
    swarm = get_airllm_swarm()
    if swarm:
        swarm.record_main_activity()
